...
1. ✅ 사용자는 자판기가 보유하고 있는 금액을 입력한다.
   + [o] "자판기가 보유하고 있는 금액을 입력해 주세요."을 출력해야 한다.
   + [o] 보유금액 입력을 받아야 한다.
      + [o] [예외] 인풋이 없는 경우
      + [o] [예외] 인풋에 숫자 외에 다른 것이 들어온 경우
      + [o] [예외] 인풋의 첫 글자가 0인 경우
...
6. ✅ 사용자가 잘못된 값을 입력할 경우 `IllegalArgumentException`를 발생시키고, "[ERROR]"로 시작하는 에러 메시지를 출력 후 해당 부분부터 다시 입력을 받는다.
